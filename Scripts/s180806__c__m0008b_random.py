...
x.alg_parsim_rand_clust(m=m0008b.Model(p0), params=params, ranges=ranges, nsims=960, jobid=8, subjobid=0, cores=32,
                        compression=0, node=int(sys.argv[1]))
x.batch_analysis(8, 0, funcs=x.all_analysis)
x.save_scores_batch(8, 0)

# Retesting in PAR-1 RNAi mode + analysis
x.alg_parsim_clust_duplicate(m=m0008b.Model(p0), changes={'ra': 0}, oldjobid=8, oldsubjobid=0, newjobid=8,
                             newsubjobid=1,
                             cores=32, node=int(sys.argv[1]), compression=0)
x.batch_analysis(8, 1, funcs=x.all_analysis)
x.save_scores_batch(8, 1)
