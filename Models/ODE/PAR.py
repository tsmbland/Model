...
class PAR:
    def __init__(self, konA, koffA, kposA, konP, koffP, kposP, ePneg, eAneg, psi, pA, pP, kAP, kPA):
    ...
    def dxdt(self, X, t):
        ...
        dA = (self.konA * Acyt) - (self.koffA * A) - (self.kAP * (P ** self.ePneg) * A) + (self.kposA * A * Acyt)
        dP = (self.konP * Pcyt) - (self.koffP * P) - (self.kPA * (A ** self.eAneg) * P) + (self.kposP * P * Pcyt)
        return [dA, dP]

    def jacobian(self, X, step=0.001):
        A = X[0]
        P = X[1]
        Acyt = self.pA - self.psi * A
        Pcyt = self.pP - self.psi * P
    ...
    def bistability_instability(self):
        ...
        if dist < 0.0001:
            # Monostable
            point = (sol1 + sol2) / 2
            w, v = np.linalg.eig(self.jacobian(point, step=0.001))
            if np.all(w < 0):
                # Stable
                return 1
            else:
            ...
            # Bistable
            w1, v1 = np.linalg.eig(self.jacobian(sol1, step=0.001))
            w2, v2 = np.linalg.eig(self.jacobian(sol2, step=0.001))
