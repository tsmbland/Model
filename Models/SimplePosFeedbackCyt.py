...
class Model:
    def __init__(self, Da, Dp, Da_cyt, Dp_cyt, konA, koffA, konP, koffP, kposA, kposP, kAP, kPA, ePneg, eAneg, xsteps,
                 Tmax, deltat, deltax, psi, am_0, ac_0, pm_0, pc_0):
        # Start concs
        self.am_0 = am_0
        self.ac_0 = ac_0
        self.pm_0 = pm_0
    ...
    def reactions(self):
        ...
        r[10] = self.Da_cyt * self.diffusion(self.ac)
        r[11] = self.Dp_cyt * self.diffusion(self.pc)
    ...
    def update_ac(self, r):
        self.ac += (r[10] + (- r[0] + r[1] - r[2] + r[3]) * self.psi) * self.deltat

    def update_pc(self, r):
        self.pc += (r[11] + (- r[5] + r[6] - r[7] + r[8]) * self.psi) * self.deltat
