...
datadirec = '/Volumes/lab-goehringn/working/Tom/ModelData/GenericPFflow2HD/'
...
def trigger_plot(ns, name):
    ...
    for i, n in enumerate(ns):
        ...
        # Plot line
        plt.plot(10 ** xpoints[order], ypoints[order] / 2, label=labels[i], c=cs[i])
    ...
    ax.set_ylabel('Minimum average \n flow velocity ' + r'$(\mu m \: s^{-1})$', linespacing=1.5)
    ax.tick_params(axis='both', labelsize=7)
    ax.set_xscale('log')
    ax.set_xlim(0.001, 10)
    ax.set_ylim(top=0.049)
...
trigger_plot([0, 3, 5], name='kant_v')
