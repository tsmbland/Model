...
class Goehring2011:
    ...
    def run(self, save_direc=None, save_gap=None, kill_uni=False, kill_stab=False, append=False):
        """
        ...
        :param kill_stab: terminate when patterns are stable. I'd advise against for phase-space diagrams, can get
            fuzzy boundaries
        :param append: if True, will append results onto existing results (if they exist). If not, will delete and
            replace
        :return:
        """
        if save_gap is None:
        ...
        if save_direc is not None:
            if not append:
                ...
                np.savetxt(save_direc + '/A.txt', np.r_[np.loadtxt(save_direc + '/A.txt'), solns[0]])
                np.savetxt(save_direc + '/P.txt', np.r_[np.loadtxt(save_direc + '/P.txt'), solns[1]])
                o = np.loadtxt(save_direc + '/times.txt')
                np.savetxt(save_direc + '/times.txt', np.r_[o, o[-1] + times])
